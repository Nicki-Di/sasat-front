...
export default function Bill({info}) {
    return (
        <div className = {"rounded bg-s-90 p-4 flex flex-col gap-4  "}>
            <div
                className = {"grid grid-rows-2 xl:justify-items-center border-b border-s-80 pb-4 " + (info.daysLeft > 0 ? "grid-cols-5" : "grid-cols-4")}>
                <p className = {"text-s-30 b1"}>دوره زمانی</p>
                <p className = {"text-s-30 b1"}>تاریخ صدور</p>
                {info.daysLeft > 0 && (<p className = {"text-s-30 b1 "}>مهلت اعتراض</p>)}
                ...
                {info.daysLeft > 0 && (<p className = {"text-s-10 b1 "}>{`${info.daysLeft}  روز مانده`}</p>)}
            ...
            <div className = {"flex flex-col gap-4 "}>
                ...
                <div className = {"rounded-3xl py-2 " + (info.daysLeft > 0 ? "bg-primary-1" : "bg-success-1")}>
                    <p className = {"b1 text-center " + (info.daysLeft > 0 ? "text-s-10" : "text-success")}>{info.daysLeft > 0 ? billMessages[1] : billMessages[0]}</p>
                </div>
            </div>
